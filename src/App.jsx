...
import { Plus, Trash2, ClipboardList, Users, Cog, BarChart3, Factory, X, Lock, Upload, Download, Search, Boxes, FileDown, ChevronDown, ChevronRight, Menu as MenuIcon, UserPlus, Mail, Chrome, Ruler, RefreshCw, Copy, Building2, Bell, ArrowLeft, Home, AlertTriangle, HelpCircle, Pencil, Check } from "lucide-react";
...
function FasonIsler({ fasonFirmalar, fasonIsler, fasonHareketler }) {
  const [baslik, setBaslik] = useState({ firmaId: "", projeKodu: "" });
  ...
  const kaydet = async () => {
    if (!baslik.firmaId) { setMsg("Firma zorunlu."); setTimeout(() => setMsg(""), 2500); return; }
    const gecerliSatirlar = satirlar.filter((r) => r.projeAdi.trim());
    if (gecerliSatirlar.length === 0) { setMsg("En az bir satıra Proje / Parça Adı girin."); setTimeout(() => setMsg(""), 2500); return; }
    const batch = writeBatch(db);
    gecerliSatirlar.forEach((r) => {
      ...
      batch.set(ref, {
        ...
        durum: "bekliyor", olusturmaTarihi: todayISO(),
      });
    });
    await batch.commit();
    setSatirlar([bosFasonSatiri()]);
    setMsg(`${gecerliSatirlar.length} satır eklendi.`); setTimeout(() => setMsg(""), 2500);
  };
  const sil = async (id) => {
      ...
      <div className="card" style={{ padding: 20 }}>
        ...
        <div style={belgeBaslikKutu}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 14 }}>
            <div>
              <label className="field-label">Firma</label>
              <select className="input" value={baslik.firmaId} onChange={(e) => setBaslik((s) => ({ ...s, firmaId: e.target.value }))}>
                <option value="">Seçin…</option>
                {fasonFirmalar.map((f) => <option key={f.id} value={f.id}>{f.ad}{f.not && f.not.trim() ? ` — ${f.not}` : ""}</option>)}
              </select>
            </div>
            <div><label className="field-label">Proje Kodu</label><input className="input" value={baslik.projeKodu} onChange={(e) => setBaslik((s) => ({ ...s, projeKodu: e.target.value }))} placeholder="Örn: PRJ-001" /></div>
          </div>
        </div>

        <div style={erpGridKutu}>
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ background: "#16232a" }}>
                  <th style={erpGridTh}>Proje / Parça Adı</th>
                  <th style={{ ...erpGridTh, width: 110 }}>Miktar</th>
                  <th style={{ ...erpGridTh, width: 130 }}>Ücret (₺)</th>
                  <th style={erpGridTh}>Resim Referansı / Link</th>
                  <th style={erpGridTh}>Açıklama</th>
                  <th style={{ ...erpGridTh, width: 36 }}></th>
                </tr>
              </thead>
              <tbody>
                {satirlar.map((r, i) => (
                  <tr key={r.key} style={{ borderBottom: "1px solid #223840" }}>
                    <td style={erpGridTd}><input className="input" style={erpGridInput} value={r.projeAdi} onChange={(e) => satirGuncelle(r.key, "projeAdi", e.target.value)} placeholder={`Satır ${i + 1}`} /></td>
                    <td style={erpGridTd}><input className="input" style={erpGridInput} value={r.miktar} onChange={(e) => satirGuncelle(r.key, "miktar", e.target.value)} /></td>
                    <td style={erpGridTd}><input className="input" type="number" step="0.01" style={erpGridInput} value={r.ucret} onChange={(e) => satirGuncelle(r.key, "ucret", e.target.value)} /></td>
                    <td style={erpGridTd}><input className="input" style={erpGridInput} value={r.resimRef} onChange={(e) => satirGuncelle(r.key, "resimRef", e.target.value)} /></td>
                    <td style={erpGridTd}><input className="input" style={erpGridInput} value={r.aciklama} onChange={(e) => satirGuncelle(r.key, "aciklama", e.target.value)} /></td>
                    <td style={{ ...erpGridTd, textAlign: "center" }}>
                      <button onClick={() => satirSil(r.key)} disabled={satirlar.length === 1} style={{ background: "none", border: "none", color: satirlar.length === 1 ? "#3a4a50" : "#6b7178", cursor: satirlar.length === 1 ? "default" : "pointer", padding: 4 }}><Trash2 size={14} /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr style={{ background: "#16232a" }}>
                  <td colSpan={2} style={{ padding: 10 }}>
                    <button className="btn-ghost" onClick={satirEkle} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12.5 }}><Plus size={13} /> Satır Ekle</button>
                  </td>
                  <td colSpan={4} style={{ padding: 10, textAlign: "right", fontSize: 13, fontWeight: 700 }}>
                    Ara Toplam: <span style={{ fontFamily: "monospace", color: "#2dd4bf" }}>{paraTR(araToplam)}</span>
                  </td>
                </tr>
              </tfoot>
            </table>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 16 }}>
          <button onClick={kaydet} disabled={fasonFirmalar.length === 0} style={{ display: "flex", alignItems: "center", gap: 7, background: "#2dd4bf", color: "#142a30", border: "none", borderRadius: 7, padding: "10px 16px", fontWeight: 700, fontSize: 13.5, cursor: "pointer" }}><Plus size={16} /> Kaydet</button>
          {msg && <span style={{ fontSize: 12.5, color: "#8b929a" }}>{msg}</span>}
        </div>
  ...
  return (
    <div style={{ display: "grid", gap: 20 }}>
      ...
      <div className="card" style={{ padding: 20 }}>
        ...
        {(arama.trim() || altGrupFiltre) ? (
          <>
            <div style={{ ...erpGridKutu, maxHeight: 340, overflowY: "auto", marginBottom: 14 }}>
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead><tr style={{ background: "#16232a" }}><th style={erpGridTh}>Stok Adı</th><th style={erpGridTh}>Stok Kodu</th><th style={erpGridTh}>Mevcut</th><th style={{ ...erpGridTh, width: 120 }}>Giriş Miktarı</th></tr></thead>
                <tbody>
