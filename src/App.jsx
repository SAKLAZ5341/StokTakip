...
function Panel({ onCikis, kullanici }) {
  ...
  useEffect(() => {
    ...
    return () => { unsub1(); unsub2(); unsub3(); unsub4(); unsub5(); unsub6(); unsub7(); unsub8(); unsub9(); unsub10(); unsub11(); unsub12(); unsub13(); unsub14(); unsub15(); };
  }, []);
        ...
        <main style={{ flex: 1, padding: 24, overflowY: "auto", minWidth: 0 }}>
          <div style={{ maxWidth: 1200, margin: "0 auto" }}>
            ...
            {tab === "satinalma-talep" && <SatinalmaTalep
              satinalmaTalepler={satinalmaTalepler} depoStok={depoStok} kullanicilar={kullanicilar} kullanici={kullanici}
            ...
            {tab === "satinalma-siparis" && <SatinalmaSiparis
              ...
              depoStok={depoStok} kullanici={kullanici}
...
const bosTalepSatiri = () => ({ key: Math.random().toString(36).slice(2), cinsi: "Stok", kodu: "", ismi: "", projeKodu: "", srmMrk: "", miktar: "", birim: "Adet", teslimTarihi: "" });
...
// ---------- Satınalma Talebi ----------
function SatinalmaTalep({ satinalmaTalepler, depoStok, kullanicilar, kullanici, siparisOlustur }) {
  const bosBaslik = () => ({
    evrakNo: "", tarih: todayISO(), belgeNo: "", belgeTarihi: "",
    proje: "", sorumlulukMerkezi: "", depo: "", talepEdenPersonel: "",
  });
  const [fisAcik, setFisAcik] = useState(false);
  const [duzenlenenId, setDuzenlenenId] = useState(null);
  ...
  const fisiTemizle = () => {
    setDuzenlenenId(null);
    setBaslik({ ...bosBaslik(), evrakNo: yeniNo(), talepEdenPersonel: kullanici?.email || "" });
  ...
  const fisiYukle = (t) => {
    ...
    setBaslik({
      ...
      proje: t.proje || "", sorumlulukMerkezi: t.sorumlulukMerkezi || "", depo: t.depo || "", talepEdenPersonel: t.talepEdenPersonel || t.talepEden || "",
    });
  ...
  const satirGuncelle = (key, alan, deger) => setSatirlar((s) => s.map((r) => (r.key === key ? { ...r, [alan]: deger } : r)));
  const satirEkle = () => setSatirlar((s) => [...s, bosTalepSatiri()]);
  const satirSil = (key) => setSatirlar((s) => (s.length > 1 ? s.filter((r) => r.key !== key) : s));
  ...
  // Lookup kaynakları
  const projeListesi = benzersizDegerler(satinalmaTalepler, "proje");
  const srmListesi = benzersizDegerler(satinalmaTalepler, "sorumlulukMerkezi");
  const depoListesi = [...new Set([
    ...satinalmaTalepler.map((t) => String(t.depo || "").trim()).filter(Boolean),
    ...depoStok.map((s) => String(s.anaGrupAdi || "").trim()).filter(Boolean),
  ])].sort().map((d) => ({ deger: d }));
  const personelListesi = [...new Set([
    ...(kullanicilar || []).map((k) => String(k.ad || k.email || "").trim()).filter(Boolean),
    ...satinalmaTalepler.map((t) => String(t.talepEdenPersonel || "").trim()).filter(Boolean),
  ])].sort().map((d) => ({ deger: d }));
  ...
  const kaydet = async () => {
    ...
    const veri = {
      ...
      proje: baslik.proje.trim(), sorumlulukMerkezi: baslik.sorumlulukMerkezi.trim(),
      depo: baslik.depo.trim(), talepEdenPersonel: baslik.talepEdenPersonel.trim(),
    ...
    try {
      ...
      if (duzenlenenId && duzenlenenId === yeniId) {
        await updateDoc(doc(db, "satinalma_talepler", duzenlenenId), veri);
        setMsg(`${baslik.evrakNo} güncellendi.`);
      } else if (duzenlenenId) {
        // Evrak No değişti: yeni numarayla oluştur, eskisini sil
        const eski = satinalmaTalepler.find((t) => t.id === duzenlenenId);
        await benzersizEvrakKaydet("satinalma_talepler", baslik.evrakNo, {
          ...
          olusturma: eski?.olusturma || Date.now(),
        });
  ...
  const yazdir = () => {
    fisYazdir(
      `Satın Alma Talep Fişi — ${baslik.evrakNo}`,
      [
        ["Evrak No", baslik.evrakNo], ["Tarih", baslik.tarih],
        ["Belge No", baslik.belgeNo], ["Belge Tarihi", baslik.belgeTarihi],
        ["Proje", baslik.proje], ["Sorumluluk merkezi", baslik.sorumlulukMerkezi],
        ["Depo", baslik.depo], ["Talep eden personel", baslik.talepEdenPersonel],
      ],
      [
        { baslik: "#", al: (r, i) => i + 1 },
        { baslik: "Cinsi", al: (r) => r.cinsi },
        { baslik: "Kodu", al: (r) => r.kodu },
        { baslik: "İsmi", al: (r) => r.ismi },
        { baslik: "Proje kodu", al: (r) => r.projeKodu },
        { baslik: "Srm.mrk.", al: (r) => r.srmMrk },
        { baslik: "Miktar", al: (r) => r.miktar, sag: true },
        { baslik: "Birim", al: (r) => r.birim },
        { baslik: "Teslim tarihi", al: (r) => r.teslimTarihi },
      ],
      satirlar.filter((r) => String(r.ismi || "").trim() || String(r.kodu || "").trim()),
      `Toplam Kalem: ${satirlar.filter((r) => String(r.ismi || "").trim() || String(r.kodu || "").trim()).length}`
    );
  };
  ...
  const disaAktar = () => excelIndir(
    satinalmaTalepler.flatMap((t) => (t.satirlar || []).map((r) => ({
      ...
      "Proje": t.proje, "Sorumluluk Merkezi": t.sorumlulukMerkezi, "Depo": t.depo, "Talep Eden Personel": t.talepEdenPersonel,
      "Cinsi": r.cinsi, "Kodu": r.kodu, "İsmi": r.ismi, "Proje Kodu": r.projeKodu, "Srm.mrk.": r.srmMrk,
    ...
    <div style={{ display: "grid", gap: 20 }}>
      ...
      >
        ...
        <div style={{ border: "1px solid #2a4b52", borderRadius: 4, padding: "12px 14px", marginBottom: 12, background: "#16232a", display: "grid", gridTemplateColumns: "minmax(250px,1fr) minmax(210px,0.85fr) minmax(300px,1.15fr)", gap: "0 24px" }}>
          ...
          {/* Sağ: Proje / Sorumluluk merkezi / Depo / Talep eden personel */}
          <div>
            <FisAlan genisEtiket etiket="Proje" deger={baslik.proje} degistir={setB("proje")} lookup
              lookupAc={() => setLookup({ baslik: "Proje Seç", secenekler: projeListesi, sec: (o) => setB("proje")(o.deger) })} />
            <FisAlan genisEtiket etiket="Sorumluluk merkezi" deger={baslik.sorumlulukMerkezi} degistir={setB("sorumlulukMerkezi")} lookup
              lookupAc={() => setLookup({ baslik: "Sorumluluk Merkezi Seç", secenekler: srmListesi, sec: (o) => setB("sorumlulukMerkezi")(o.deger) })} />
            <FisAlan genisEtiket etiket="Depo" deger={baslik.depo} degistir={setB("depo")} lookup
              lookupAc={() => setLookup({ baslik: "Depo Seç", secenekler: depoListesi, sec: (o) => setB("depo")(o.deger) })} />
            <div style={{ marginBottom: -7 }}>
              <FisAlan genisEtiket etiket="Talep eden personel" deger={baslik.talepEdenPersonel} degistir={setB("talepEdenPersonel")} lookup
                lookupAc={() => setLookup({ baslik: "Personel Seç", secenekler: personelListesi, sec: (o) => setB("talepEdenPersonel")(o.deger) })} />
            </div>
          </div>
        </div>

        {/* ---- Satır grid'i: Cinsi | Kodu | İsmi | Proje kodu | Srm.mrk. | Miktar | Birim | Teslim tarihi ---- */}
        <div style={{ border: "1px solid #2a4b52", borderRadius: 4, overflow: "hidden" }}>
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 960 }}>
              <thead>
                <tr>
                  ...
                  <th style={{ ...fisGridTh, width: 120 }}>Proje kodu</th>
                  <th style={{ ...fisGridTh, width: 110 }}>Srm.mrk.</th>
              ...
              <tbody>
                {satirlar.map((r, i) => (
                  <tr key={r.key}>
                    ...
                    <td style={fisGridTd}><input style={fisHucreInput} value={r.projeKodu} onChange={(e) => satirGuncelle(r.key, "projeKodu", e.target.value)} /></td>
                    <td style={fisGridTd}><input style={fisHucreInput} value={r.srmMrk} onChange={(e) => satirGuncelle(r.key, "srmMrk", e.target.value)} /></td>
      ...
      <div className="card" style={{ padding: 0, overflow: "hidden" }}>
        ...
        <div style={{ overflowX: "auto", maxHeight: 620, overflowY: "auto" }}>
          <table>
            <thead><tr><th style={{ width: 36 }}><input type="checkbox" checked={hepsiSecili} onChange={tumunuSecToggle} /></th><th>Evrak No</th><th>Tarih</th><th>Talep Eden</th><th>Proje</th><th>Depo</th><th>Kalem</th><th>Durum</th><th>Sipariş No</th><th></th></tr></thead>
            <tbody>
              ...
              {filtrelenmis.map((t) => {
                ...
                return (
                  <tr key={t.id}>
                    ...
                    <td>
                      <button onClick={() => fisiYukle(t)} title="Fişi aç" style={{ background: "none", border: "none", padding: 0, fontFamily: "monospace", fontWeight: 700, color: "#2dd4bf", cursor: "pointer", textDecoration: "underline" }}>{t.evrakNo}</button>
                    </td>
...
// ---------- Satınalma Siparişi ----------
function SatinalmaSiparis({ satinalmaSiparisler, satinalmaTalepler, depoStok, kullanici, taslak, taslakTemizle }) {
  const [fisAcik, setFisAcik] = useState(false);
  ...
  const sil = async (s) => {
    if (!window.confirm(`${s.evrakNo} numaralı sipariş silinecek. Emin misiniz?`)) return;
  ...
  const secilenleriSil = async () => {
    if (secililer.size === 0) return;
    if (!window.confirm(`${secililer.size} sipariş kalıcı olarak silinecek. Emin misiniz?`)) return;
    setTopluDurum("Siliniyor…");
    const idler = [...secililer];
    for (let i = 0; i < idler.length; i += 400) {
      const batch = writeBatch(db);
  ...
  return (
    <div style={{ display: "grid", gap: 20 }}>
      ...
      <EvrakPenceresi
        acik={fisAcik} kapat={() => setFisAcik(false)}
        baslik={baslik.talepEvrakNo ? `Satınalma Sipariş Fişi — ${baslik.talepEvrakNo} talebinden` : "Satınalma Sipariş Fişi"}
      ...
      >
        <div style={{ border: "1px solid #2a4b52", borderRadius: 4, padding: "12px 14px", marginBottom: 12, background: "#16232a", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: "0 26px" }}>
          ...
          <div>
            <div style={fisSatir}><span style={fisEtiket}>Tedarikçi</span><input style={fisInput} value={baslik.tedarikci} onChange={(e) => setBaslik((s) => ({ ...s, tedarikci: e.target.value }))} /></div>
          ...
          <table>
            ...
            <tbody>
              ...
              {filtrelenmis.map((s) => (
                <tr key={s.id}>
                  <td><input type="checkbox" checked={secililer.has(s.id)} onChange={() => birSecToggle(s.id)} /></td>
                  <td style={{ fontFamily: "monospace", fontWeight: 700, color: "#2dd4bf" }}>{s.evrakNo}</td>
                  ...
                  <td><button onClick={() => sil(s)} style={{ background: "none", border: "none", color: "#6b7178", cursor: "pointer", padding: 4 }}><Trash2 size={14} /></button></td>
                </tr>
              ))}
            </tbody>
          </table>
